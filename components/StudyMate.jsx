...
export default function StudyMate() {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      if (user) {
        setLoginUser({ name: user.displayName || "사용자", email: user.email, avatar: user.displayName?.[0] || "U" });
        // Firestore에서 사용자 데이터 불러오기
        const snap = await getDoc(doc(db, "users", user.uid));
        if (snap.exists()) {
          const data = snap.data();
          if (data.children) setChildren(data.children);
          if (data.activeChildId) setActiveChildId(data.activeChildId);
          if (data.allRadar) setAllRadar(data.allRadar);
          if (data.allQuests) setAllQuests(data.allQuests);
  ...
  const saveToFirestore = async (updates = {}) => {
    ...
    try {
      await setDoc(doc(db, "users", auth.currentUser.uid), {
        children, activeChildId, allRadar, allQuests, wallets, schedules,
        ...updates,
  ...
  const toggleQuest = (i) => {
    setQuests(prev => {
      ...
      setTimeout(() => saveToFirestore(), 500);
      return next;
    });
  };
  ...
  const LoginScreen = () => (
    <div style={{ minHeight:"100vh", background:R.bg, display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"'Pretendard',sans-serif" }}>
      ...
      <div className="card fadein" style={{ width:360, padding:"44px 36px", textAlign:"center" }}>
        ...
        {/* 구글 */}
        <button className="btn" onClick={handleGoogleLogin} style={{
        ...
        {/* 카카오 */}
        <button className="btn" onClick={() => alert("카카오 로그인은 준비 중이에요! 구글 로그인을 이용해주세요 😊")} style={{
        ...
        {/* 네이버 */}
        <button className="btn" onClick={() => alert("네이버 로그인은 준비 중이에요! 구글 로그인을 이용해주세요 😊")} style={{
    ...
    <div className="fadein" style={{display:"flex",flexDirection:"column",gap:14}}>
      ...
      <div className="card" style={{padding:"20px 22px"}}>
        <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:16}}>
          ...
          <button onClick={handleLogout} className="btn" style={{marginLeft:"auto",background:R.redA,border:`1px solid ${R.redBd}`,color:R.red9,padding:"6px 12px",fontSize:11,fontWeight:700}}>로그아웃</button>
        </div>
      </div>
  ...
  if (screen === "loading") return (
    <div style={{ minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", background:R.bg }}>
      ...
      <div style={{ textAlign:"center" }}>
        ...
        <p style={{ fontSize:16, fontWeight:700, color:R.muted }}>StudyMate 로딩 중...</p>
      </div>
    </div>
  );
